...
class DonorAuthTest(BaseCrowdFundTestCase):
    ...
    def test_donor_registration_view(self):
        ...
        self.assertTrue(
            any("account has been created successfully" in str(msg).lower() for msg in messages),
...
class DonorProfileTest(BaseCrowdFundTestCase):
    ...
    def test_edit_profile_form(self):
        ...
        # Check that donor profile was updated
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any("profile" in str(msg).lower() and "updated" in str(msg).lower() for msg in messages),
            "Profile updated success message should be displayed"
        )
...
class DonorCampaignBrowsingTest(BaseCrowdFundTestCase):
    ...
    def test_donor_can_view_organizations(self):
        ...
        # Check that organizations are listed
        self.assertContains(response, 'Test Organisation')
        self.assertContains(response, 'Test mission statement')
...
class DonorDonationTest(BaseCrowdFundTestCase):
    ...
    def test_donor_can_view_campaign_detail(self):
        ...
        response = self.client.get(reverse('campaigns:detail', kwargs={'pk': self.campaign.id}))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Test Campaign')
        self.assertContains(response, 'A test campaign for donations')
    ...
    def test_donor_can_make_donation(self):
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any("thank you for your donation" in str(msg).lower() for msg in messages),
